...
import { TokenType, TokenIndex as NovaTokenIndex } from '../tokens/index.mjs'
import { splitCharRef } from './split-char-ref.mjs'
import { contentMap } from './content-map.mjs'
import { FAIL, errorToken } from './tokens.mjs'
import { LegacyTokenIndex } from './token-index-override.mjs'
 ...
 * @property {boolean} [useLegacyTokens]
 */
...
export class Lexer {
  ...
  /** @type {typeof LegacyTokenIndex | typeof NovaTokenIndex} */
  #tokens

  constructor(/** @type {LexerOptions | undefined} */ options) {
    ...
    if (options?.useLegacyTokens) {
      this.#tokens = LegacyTokenIndex
    } else {
      this.#tokens = NovaTokenIndex
    }
  }

  /** @returns {DisposeCallback} */
  onWrite(
    /** @type {OnWriteCallback} */ callback
  ) {
  ...
  /** @type {LexerResults} */
    const result = []
    this.onWrite((tokens) => result.push(tokens))
    this.write(code)
    this.end()
    return result
  }
  ...
  #emit(
    ...
    if (type === errorToken) {
      ...
      this.#entry = S.Bogus
      const label = this.#tokens[TokenType.bogusStart]
      // @ts-expect-error TODO seperate Lexer
      this.#triggerWriteCallback([label, '<!'])
      this.#triggerWriteCallback([
        this.#tokens[TokenType.bogusData],
    ...
    const name = Reflect.get(this.#tokens, type)
    this.#triggerWriteCallback([
      name,
      this.#buffer.substring(this.#anchor, end),
    ])
  ...
  #triggerWriteCallback(/** @type {LexerResult} */ value) {
