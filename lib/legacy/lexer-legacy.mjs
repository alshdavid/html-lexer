...
import { TokenType as T, TokenIndex } from "../tokens/index.mjs";
...
function Lexer(delegate) {
  ...
  function write(input) {
    ...
    while (pos < length) {
      ...
      if (end < buffer.length || closed) emit(table[exit][0], anchor, end);
      else {
        pos = end = anchor;
        break;
      }
  ...
  function emit(type, anchor_, end_) {
    ...
    switch (type) {
      ...
      case T.mDeclStart: {
        ...
        delegate.write([
          names[T.bogusData],
          buffer.substring(anchor + 2, end_),
        ]);
        return (anchor = pos = end_); // NB returns
      }
      ...
      case T.tagEnd: {
        ...
        entry =
          lastTagType === T.startTagStart && !xmlIsh
            ? contentMap[lastStartTagName] || S.Main
            : S.Main;
      ...
      case T.unquoted:
        ...
        break;
      case T.commentStart:
        entry = S.BeforeCommentData;
        break;
      case T.commentData:
        entry = S.InCommentData;
        break;
      case T.commentEnd:
        entry = S.Main;
        break;
...
function splitCharRef(string, entry, nextChar) {
  ...
  if (PREFIXED.test(string)) return [["charRefNamed", string]];
  ...
  if (!r)
    return terminated ? [["charRefNamed", string]] : [[dataTokenType, string]];
